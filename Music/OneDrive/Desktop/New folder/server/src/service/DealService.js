...
class DealService {
    ...
    async updateDeal(id, updateData) {
        ...
        // ✅ Domain logic: prevent invalid discount
        if (updateData.discount < 0 || updateData.discount > 100) {
            throw new AppError("Discount must be 0-100%", 400);
        }

        return await Deal.findByIdAndUpdate(id, { discount: updateData.discount }, { new: true }).populate('category');
    }
